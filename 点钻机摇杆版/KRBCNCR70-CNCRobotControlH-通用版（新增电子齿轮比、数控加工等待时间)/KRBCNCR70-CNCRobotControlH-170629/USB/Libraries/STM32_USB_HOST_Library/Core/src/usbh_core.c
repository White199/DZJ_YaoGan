...
* @retval None
*/
void USBH_Process(void)	  //usb处理函数
{
  volatile USBH_Status status = USBH_FAIL;			 //USB状态标志变量
    
  switch (USBH_Machine.gState)				 
  {
	  case HOST_ISSUE_CORE_RESET :	 // HOST_ISSUE_CORE_RESET=1  USB拔出和插入时进入复位,再进入总线空闲状态 
	  ...
	  case HOST_IDLE :	     //开机进入检测是否连接  HOST_IDLE=0  USB空闲状态,如果USB未插入,都处于此状态
		    if (HCD_IsDeviceConnected(&USB_OTG_FS_dev)) //检测USB是否连接,连接之后进入连接状态
		    {
	  ...
	  case HOST_DEV_ATTACHED :	//HOST_DEV_ATTACHED=2 	进入连接状态后就由USBH_Machine状态机变量调用相关处理函数-->usb_usr.c   
		    USBH_Machine.usr_cb->DeviceAttached();
		    USBH_Machine.conn_cb->DisconnStatus = 0; 
		    USBH_Machine.conn_cb->ConnHandled = 1;  
		    
		    /* Reset USB Device */
		    if ( HCD_ResetPort(&USB_OTG_FS_dev) == 0)	//复位USB设备
		    {
		    ...
		    {
		      USBH_Machine.gState = HOST_ENUMERATION;				        //切换至枚举状态
		      USBH_Machine.usr_cb->DeviceSpeedDetected(USBH_Device.speed);	//检测USB设备速度
	  ...
	  //USBH进入枚举状态   6
	  case HOST_ENUMERATION:       //直接到HOST_CTRL_XFER=9了！！！	USBH_HandleEnum()函数返回错误,枚举失败
		    /* Check for enumeration status */  
		    if ( USBH_HandleEnum(&USB_OTG_FS_dev) == USBH_OK)	 //枚举处理函数	获得USB设备基本信息：设备描述符,配置描述符等
		    { 													 //基本信息：设备类型,ID好	USBH_HandleEnum()
		      /* The function shall return USBH_OK when full enumeration is complete */		          
		      USBH_Machine.gState  = HOST_USR_INPUT;    
		    }
		    break;
    
	  case HOST_USR_INPUT:    //10 加载设备驱动并初始化
	  ...
	  //
	  case HOST_CLASS_REQUEST:    //7 初始化设备驱动类的状态机
	  ...
	  case HOST_CLASS:   			//8
		    ...
		    status = USBH_Machine.class_cb->Machine(&USB_OTG_FS_dev , &USBH_Device);  //最终调用USBH_MSC_Handle()函数 位于ubsh_msc_core.c中
		    USBH_ErrorHandle(status);
		    break;       
    
	  case HOST_CTRL_XFER:			//9
...
USBH_Status USBH_HandleControl (USB_OTG_CORE_HANDLE *pdev)
{
  ...
  case CTRL_SETUP_WAIT:		  //2
    /* case SETUP packet sent successfully */
    if(URB_Status == URB_DONE)	  //1
    { 
    ...
    else if(URB_Status == URB_ERROR)	   //3
    {

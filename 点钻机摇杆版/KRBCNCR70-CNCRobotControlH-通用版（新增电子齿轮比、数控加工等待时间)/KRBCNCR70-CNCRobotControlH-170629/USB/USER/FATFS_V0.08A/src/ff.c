...
FRESULT f_write (
	FIL *fp,			/* Pointer to the file object */	    //文件对象结构指针
	const void *buff,	/* Pointer to the data to be written */	//存储写入指针的数据缓冲区的指针
	UINT btw,			/* Number of bytes to write */			//要写入的字节数变量
	UINT *bw			/* Pointer to number of bytes written *///返回已写入字节数变量的指针
)
{
	FRESULT res;
	DWORD clst, sect;
